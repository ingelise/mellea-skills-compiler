---
name: mellea-fy
description: '# Melleafy: Decompose an Agent Spec into Mellea Code'
metadata:
  user-invocable: true
...
**Rule OUT-2 — Package name.** The wrapper computes `<package_name>` and injects it into the system prompt. Use the injected value verbatim. The derivation logic lives in `compile/compiler.py::_derive_mellea_package_name`.
